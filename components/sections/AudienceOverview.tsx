...
import { KPISkeletons } from "@/components/ui/Skeleton";
import { scaledKPI } from "@/data/mockData";
...
export function AudienceOverview() {
  ...
  const kpi = scaledKPI(rawKpi, dateRange.label);
  ...
  const cards: KpiCard[] = [
    ...
    { label: "Follower Growth (MTM)", value: `+${kpi.followerGrowth.toLocaleString("en-IN")}`, badge: `+${kpi.mom.followerGrowth}%`, trend: [20, 24, 22, 28, 26, 32, 30, 36, 34, 40, 44, 48] },
  ...
  return (
    <Section
      ...
      action={<AIInsightButton section="Audience Overview" data={kpi} autoFetch />}
    >
      {!loaded ? (
        <KPISkeletons />
      ) : (
